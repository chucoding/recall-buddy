...
export const getCommits = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      const response = await fetch(`https://api.github.com/repos/${repoPath}/commits?since=${since}&until=${until}`, {
        headers: {
...
export const getMarkdown = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      const response = await fetch(`https://api.github.com/repos/${repoPath}/contents/${filename}`, {
        headers: {
